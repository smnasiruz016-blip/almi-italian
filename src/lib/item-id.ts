...
import AUDIO_MANIFEST_JSON from "@/data/ascolto-audio.json";

/** The pre-rendered ASCOLTO clips, generated by scripts/audio/render-ascolto.mts. Imported as
 *  data so no filesystem read happens on a request. */
const AUDIO_MANIFEST: Record<string, { url: string }> = AUDIO_MANIFEST_JSON;
...
export function toRunnerItem(it: BankItem): RunnerItem {
  return {
    ...
    // Looked up, never derived-and-hoped: an item gets an audioUrl only if the manifest has
    // an entry, and scripts/gates/ascolto-audio-gate.mts proves the manifest matches the
    // files on disk in BOTH directions.
    audioUrl: AUDIO_MANIFEST[stableItemId(it)]?.url,
