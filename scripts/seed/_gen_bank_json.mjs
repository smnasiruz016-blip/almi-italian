...
if (drift && drift.length > 0 && process.env[OVERRIDE] !== OVERRIDE_VALUE) {
  console.error("");
  console.error("🔴 REFUSING TO REGENERATE — the seed and the shipped bundle disagree.");
  console.error("");
  console.error(`   ${drift.length} item(s) differ between scripts/seed/batch1/ and src/data/items-batch1.json.`);
  console.error("   This script writes the bundle FROM the seed. Running it now would overwrite what");
  console.error("   ships with whatever the seed currently says — including any fix that landed on the");
  console.error("   bundle and never reached the seed.");
  console.error("");
  console.error("   That is not hypothetical; it is why this guard exists. The seed spent months 16");
  console.error("   items behind the bundle (#38 widened 15 writing windows to 80-120, #39 added the");
  console.error("   dialogue dashes to one ASCOLTO script) and running this script would have reverted");
  console.error("   both in silence. PR-E synced the seed, which is why the drift is normally zero and");
  console.error("   this message is normally unreachable. You are reading it, so the two sides have come");
  console.error("   apart again — and WHICH SIDE IS RIGHT is a question, not an assumption. Last time it");
  console.error("   was the bundle. Check before you pick.");
  console.error("");
  console.error("   The history and the resolution are in docs/seed-bundle-drift.md.");
  console.error("");
  console.error("   THE FIX IS TO MAKE THE TWO SIDES AGREE, not to bypass this. gate:seed-bundle fails");
  console.error("   on any drift, so bypassing here only moves the failure to the build.");
  console.error("");
