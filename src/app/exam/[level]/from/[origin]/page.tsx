...
export default async function Page({ params }: { params: Promise<{ level: string; origin: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-12">
      <p className="text-xs font-semibold uppercase tracking-widest text-almi-coral-text">{l.exam} · {o.name}</p>
      ...
      <div className="mt-6 flex flex-wrap gap-3 text-sm">
        <Link href="/practice" className="rounded-full bg-almi-coral px-5 py-2 font-semibold text-almi-ink hover:bg-almi-coral-deep">Practise {l.name}</Link>
