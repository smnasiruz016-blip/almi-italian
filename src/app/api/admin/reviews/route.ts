...
export async function PATCH(req: Request): Promise<NextResponse> {
  ...
  if (!user || !canAccessAdmin(user.email)) {
    // An admin route being probed is worth a line whether or not the caller had a session.
    logRefusal({ route: "/api/admin/reviews", status: 403, reason: user ? "not-admin" : "no-session", req, ...(user ? { userId: user.id } : {}) });
