...
//
// ── WHAT WAS WRONG ──
// Five routes refused requests with 401/403 and logged NOTHING: /api/auth/login,
// /api/billing/checkout, /api/billing/portal, /api/admin/stats and /api/admin/reviews. A
// credential-stuffing run against login, or someone walking the admin routes, left no trace at
// all — there was nothing to notice, and nothing to look at afterwards. Separately, the handful
// of routes that did log used bare `console.error` with a prose prefix, so nothing could be
// filtered or counted.
//
// /api/it/submit is the sharpest case: its reply discloses the correct answer for every atom
// posted, so a run of 401s or 402s against it is somebody harvesting the answer key.
//
...
//     Italian writing. `redactMessage` scrubs anything key-shaped that reached a message anyway.
//
...
function redactMessage(msg: string): string {
  return msg
    ...
    // `\S+` alone is not enough: "authorization=Bearer abc.def.ghi" has a SPACE in the value, so
    // the old pattern consumed "Bearer" and published the token after it. Scheme words are eaten
    // explicitly, then the rest of the value to the end of the line — a credential is the last
    // thing that should be truncated by a lazy quantifier. Found by the security gate, not review.
    .replace(/\b(password|passwd|token|secret|api[-_]?key|authorization|cookie)\s*[:=]\s*(?:bearer|basic|token)?\s*[^\s,;]+.*$/gim, "$1=[redacted]")
    .slice(0, 600);
}
