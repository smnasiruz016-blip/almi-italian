...
// crucially it works even before Stripe is wired (fail-closed billing must not block the trial).
// NB this is an ADDITIVE divergence from the AlmiPrep/AlmiSpanish canonical gate, where the only
// "trial" is Stripe's own trialing status (card required). See the practice gate below.
