  ...
  return (
    <div className="mt-4">
      {hasSubscription ? (
        ...
        <button onClick={() => go("/api/billing/checkout", setBusy, setErr)} disabled={busy} className="rounded-full bg-almi-coral px-6 py-2.5 font-semibold text-almi-ink hover:bg-almi-coral-deep disabled:opacity-60">
          {busy ? "Starting…" : "Start 7-day free trial — $12/mo"}
        </button>
      )}
      {err && <p className="mt-2 text-sm text-almi-coral-text">{err}</p>}
    </div>
  );
}
